...
public class Board {
    ...
    public void addRandomTile() {
        if (openSpaces.size() > 0) {
            ...
            int randomValue = StdRandom.uniform() < 0.1 ? 4 : 2;
            gameBoard[randomSpot.getRow()][randomSpot.getCol()] = randomValue;
    ...
    public void swipeLeft() {
        ...
        for (int i = 0; i < gameBoard.length; i++) {
            ...
            for (int j = 0; j < gameBoard[i].length; j++) {
                if (gameBoard[i][j] != 0) {
                if (j != currentIndex) {
            gameBoard[i][currentIndex] = gameBoard[i][j];
            gameBoard[i][j] = 0;
        }
        currentIndex++;
        }
    ...
    public void flipRows() {
        // WRITE YOUR CODE HERE
        int rowCount = this.gameBoard.length;
        int colCount = this.gameBoard[0].length;
        int[][] newGameboard = new int[rowCount][colCount];
        for (int i = 0; i < rowCount; i++) {
            for (int j = 0; j < colCount; j++) {
                newGameboard[i][j] = this.gameBoard[rowCount - i - 1][j];
            }
        }
    ...
    public void makeMove(char letter) {
        // Transpose the board for up and down movements
            if (letter == 'U' || letter == 'D') {
                transpose();
            }
        
            // Flip the rows for right movements
            if (letter == 'R') {
                rotateBoard();
                rotateBoard();
                swipeLeft();
            ...
            // Flip the rows back for right movements
            if (letter == 'R') {
                rotateBoard();
                rotateBoard();
                swipeLeft();
            ...
            // Transpose the board back for up and down movements
            if (letter == 'U' || letter == 'D') {
                transpose();
            }
        
            // Flip the rows for down movements
            if (letter == 'D') {
                flipRows();
